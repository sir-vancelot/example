...
/**
 * The [BatmanDBDatastore] is a class which implements the [C.R.U.D](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete)
 * database model as well as some more specific read functions. A [BatmanDBDatastore] requires a
 * [dbOpenHelper], [batmanQuery], and [mapper] for construction.
 */
open class BatmanDBDatastore<Model : BatmanModel>(private val dbOpenHelper: SQLiteOpenHelper,
                                                  private val batmanQuery: BatmanDBQuery<Model>,
                                                  private val mapper: BatmanModelMapper<Model>) {
    ...
    /**
     * Used to delete a database entry by its id.
     */
    open fun delete(id: String) {
        ...
        synchronized(cacheList as Any) {
            cacheList.remove(cacheList.first{ it.id.unique == id })
        }
